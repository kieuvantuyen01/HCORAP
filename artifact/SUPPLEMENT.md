...
The [analysis snapshot](results/) contains machine-readable tables
for the reported results. Its [manifest](results/manifest.json)
...
[research-depth implementation guide](../docs/RESEARCH_DEPTH_IMPLEMENTATION_20260909.md).
...
[encoding runbook](../docs/COMPACT_RESULTS_RUNBOOK.md).
...
The [eight-configuration summary](results/preliminary/factorial_summary.csv)
and [conditional comparisons](results/preliminary/factorial_contrasts.csv)
...
See the [paired policy results](results/policy/corrected_pairwise_pairs.csv)
and [summary](results/policy/corrected_pairwise_summary.csv).
...
setting. The [per-instance intervals](results/diagnostics/weighted_face_intervals.csv)
...
[budget curves](results/diagnostics/continuity_budget_curves.csv)
and [adjacent changes](results/diagnostics/continuity_budget_changes.csv)
...
[cell summaries](results/load/load_cell_summary.csv) and
[matched pairs](results/load/load_policy_pairs.csv) also record
...
runtimes. The [figure statistics](results/encoding/compact_figure_statistics.json)
retain every percentage and the unrounded confidence limits. The
[paired runtimes](results/encoding/policy_encoding_pairs.csv) retain
...
completed runs. See the [cross-solver pairs](results/commercial/cross_solver_pairs.csv).
...
The [GCP run instructions](../docs/RESEARCH_DEPTH_IMPLEMENTATION_20260909.md) cover
dependencies, licenses, input archives, configuration, and resuming a campaign.
The [compact-results runbook](../docs/COMPACT_RESULTS_RUNBOOK.md) covers the original
policy, encoding, and commercial comparisons. The checksums and expected row
counts for the delivered analyses are recorded in the
[artifact manifest](results/manifest.json).
