...
def _result_counts(optimal: Any, infeasible: Any, timed_out: Any) -> str:
    optimal_runs = _integer(optimal)
    infeasible_runs = _integer(infeasible)
    timeout_runs = _integer(timed_out)
    counts = f"{optimal_runs} / {infeasible_runs} / {timeout_runs}"
    return rf"\textbf{{{counts}}}" if timeout_runs == 0 else counts
...
def generate(
    ...
    table_lines = [
        "% Generated by experiments/generate_compact_manuscript_results.py.",
        r"\begin{table}[tbp]",
        r"\caption{EvalMaxSAT performance by policy and encoding on 48 Original instances.}",
    ...
    for method in POLICIES:
        ...
        status_s = _result_counts(
            sn_row["optimum_runs"], sn_row["unsat_runs"], sn_row["timeout_runs"]
        )
        status_t = _result_counts(
            tot_row["optimum_runs"], tot_row["unsat_runs"], tot_row["timeout_runs"]
        )
        ...
        table_lines.append(
            f"{_policy_label(method)} & SN & {status_s} & "
            f"{par2_s} & {med_s}\\\\"
        )
        table_lines.append(
            f"{_policy_label(method)} & TOT & {status_t} & "
            f"{par2_t} & {med_t}\\\\"
        )
    ...
    for backend in expected_backends:
        for method in POLICIES:
            ...
            result_counts = _result_counts(
                row["optimal_runs"], row["infeasible_runs"], row["timeout_runs"]
            )
