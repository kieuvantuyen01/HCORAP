#!/usr/bin/env python3
"""Quantify objective-vector, assignment, and penalty-ratio sensitivity over weight grids."""
...
from research_depth_common import resolve_instance  # noqa: E402


ALLOWED = {"OPTIMUM", "UNSAT", "UNSATISFIABLE", "INFEASIBLE", "TIMEOUT", "TIMEOUT_FEASIBLE"}
...
def analyze(result_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if any(row["method"] != "weighted" for row in rows):
        raise ValueError("weight sensitivity requires a weighted-only campaign")
    optimum = [row for row in rows if row["status"] == "OPTIMUM"]
    enriched = []
    for row in optimum:
        instance_hash = row["instance_sha256"]
        if instance_hash not in penalties:
            penalties[instance_hash] = read_instance(resolve_instance(row["instance"], instance_hash)).penalty
    ...
    instance_summary = []
    penalty_ratio_groups_total = 0
    penalty_ratio_groups_single_vector = 0
    for instance_hash, group in sorted(by_instance.items()):
        ...
        by_penalty_ratio: dict[tuple[int, int], list[dict[str, Any]]] = defaultdict(list)
        for row in group:
            by_penalty_ratio[(row["normalized_wc"], row["normalized_wo"])].append(row)
        repeated_penalty_ratio_groups = [items for items in by_penalty_ratio.values() if len(items) >= 2]
        stable_penalty_ratio_groups = sum(
            len({item["vector"] for item in items}) == 1
            for items in repeated_penalty_ratio_groups
        )
        penalty_ratio_groups_total += len(repeated_penalty_ratio_groups)
        penalty_ratio_groups_single_vector += stable_penalty_ratio_groups
        ordered = sorted(
            group,
            key=lambda item: (
                item["effective_continuity_to_overtime_ratio"] is None,
                item["effective_continuity_to_overtime_ratio"] or 0,
                item["wc"], item["wo"],
            ),
        )
        transitions = sum(
        ...
        instance_summary.append(
            {
                ...
                "repeated_penalty_ratio_groups": len(repeated_penalty_ratio_groups),
                "penalty_ratio_groups_with_one_vector": stable_penalty_ratio_groups,
            }
        )
        ...
        weight_summary.append(
            {
                ...
                "median_effective_continuity_to_overtime_ratio": statistics.median(
                    ...
                    if row["effective_continuity_to_overtime_ratio"] is not None
                ) if any(row["effective_continuity_to_overtime_ratio"] is not None for row in group) else None,
            }
        )
    ...
    result = {
        "interpretation": "SIM coefficient remains 1. Equal wc:wo ratios are not equivalent objectives; these counts measure observed sensitivity to the common penalty multiplier. Returned assignments can differ through tie-breaking.",
        ...
        "unverified_optimum": sum(
            row["status"] == "OPTIMUM" and row["verified"].lower() != "true" for row in rows
        ),
        ...
        "repeated_penalty_ratio_groups": penalty_ratio_groups_total,
        "penalty_ratio_groups_with_one_vector": penalty_ratio_groups_single_vector,
    }
