...
Từ đó, ma trận thực nghiệm chính được thu hẹp từ 50 xuống:
\begin{equation}
...
\begin{tabularx}{\textwidth}{@{}c>{\ttfamily}p{3.5cm}>{\ttfamily}p{2.5cm}>{\ttfamily}p{3cm}Y@{}}
\toprule
...
Script \path{run_main_8cfg.sh} chạy tuần tự toàn bộ 800 instances
(400 \texttt{U30} + 400 \texttt{U40}), ghi JSON từng run vào
\path{cfg}$k$/\textit{instance}\path{.json}. Script
\path{collect_main_results.py} tạo ba file CSV:
\path{results_per_instance.csv} (1~dòng/run),
\path{summary_by_config.csv} (1~dòng/cấu hình, có PAR-2 và OPTIMUM\%), và
\path{comparison_pivot.csv} (1~dòng/instance, so sánh 8~cfg song song).
Các cột CSV bao gồm tối thiểu:
\texttt{instance\_name}, \texttt{status}, \texttt{elapsed\_s},
\texttt{variables}, \texttt{hard\_clauses}, \texttt{soft\_clauses},
\texttt{best\_value}, \texttt{similarity}, \texttt{continuity},
\texttt{overtime}, \texttt{coverage}.
Không tái sử dụng result directory khi schema hoặc campaign thay đổi.
Chạy lại từ điểm dừng bằng cờ \texttt{-{}-resume}.
