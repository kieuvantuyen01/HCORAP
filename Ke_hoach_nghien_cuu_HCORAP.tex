...
Điểm $\delta=0$ không được diễn giải là tương đương với B1
continuity-priority. Khi $\delta=0$, $L_0=\SIM^*$ nên B2 có thứ tự ưu tiên hiệu
dụng $\max\SIM\succ\min\CONT\succ\min\OT$; tầng tối đa hóa similarity cuối là
dư thừa vì similarity đã bị khóa ở optimum toàn cục. Ngược lại, B1
continuity-priority dùng
$\min\CONT\succ\max\SIM\succ\min\OT$. Vì vậy hai policy có thể trả về các
nghiệm khác nhau. Trên regression instance trade-off hiện tại, B1
continuity-priority cho $(\SIM,\CONT,\OT)=(8,0,1)$, trong khi B2 với
$\delta=0$ cho $(9,1,0)$.
...
Do $\SIM$ nhận giá trị nguyên và $L_\delta$ dùng ceiling, ánh xạ từ $\delta$
sang $L_\delta$ là many-to-one: nhiều mức $\delta$ có thể tạo cùng miền khả
thi và cùng $(\SIM,\CONT,\OT)$. Phân tích phải báo cáo riêng số mức $\delta$ đã
chạy, số lower bound phân biệt và số objective point duy nhất; các điểm trùng
không được xem là các quan sát Pareto độc lập. Bảng raw vẫn giữ mọi run để phân
tích thời gian, còn bảng unique points gộp theo instance/cấu hình và lưu danh
sách $\delta$ nguồn. Nếu còn budget tính toán, chèn thêm mức $\delta$ vào khoảng
có gap lớn nhất trong objective space \cite{mesquita2023}.
...
156 regression tests đã qua; smoke campaign nhỏ gồm 200 top-level runs trên đủ
...
\paragraph{Pilot 50 cấu hình và kết quả chọn lọ.}
Pilot đã được chạy trên 4~instance (\texttt{inst\_1}, \texttt{inst\_5},
\texttt{inst\_6}, \texttt{inst\_10}) với solver \textbf{Open-WBO}
(build từ source, macOS arm64), timeout 60\,giây, method \texttt{weighted}.
Kết quả tổng hợp 200 lần chạy cho thấy:
\begin{itemize}
  \item \texttt{both-plus} chỉ đạt 67\% OPTIMUM do timeout trên
    \texttt{inst\_10} với cả hai cardinality encoding ---\
    cấu hình này bị loại khỏi thực nghiệm chính.
  \item IC \texttt{none} và \texttt{both}: 100\% OPTIMUM, độ tin cậy cao nhất
    trong nhóm IC được giữ lại.
  \item Symmetry breaking không tạo ra sự khác biệt có ý nghĩa trên
    instance nhỏ; giữ \texttt{none} (baseline) và
    \texttt{slot-service} (kết hợp hai loại SB) làm đại diện.
\end{itemize}

Từ đó, ma trận thực nghiệm chính được thu hẹp từ 50 xuống 8 cấu hình cố định
(xem Bảng~\ref{tab:8-cfg-main} trong \S\ref{sec:matrix-cfg}):
\begin{equation}
  2\times2\times2=8\ \text{cấu hình} \quad (\text{bao gồm ORIGINAL = cfg1}).
  \label{eq:8-config-matrix}
\end{equation}
...
Implementation C++ dùng một policy tách khỏi B0/B1. Tham số $\delta$ được kiểm
tra trước khi gọi solver và được phân tích dưới dạng phân số thập phân chính
xác. Mỗi run ghi $\SIM^*$, $L_\delta$, realized similarity loss và optimum của
từng tầng. Sau tầng tie-break, verifier đối chiếu metric cuối với
$L_\delta$, $\SIM^*$, $\CONT^*$, $\OT^*$ và optimum similarity cuối; kiểm tra
này xác nhận tính nhất quán của chuỗi nghiệm nhưng không thay thế proof
certificate optimality của backend.
...
\label{sec:matrix-cfg}

Từ kết quả pilot (Eq.~\ref{eq:8-config-matrix}), ma trận thực nghiệm chính
sử dụng \textbf{8 cấu hình} cố định:

\begin{table}[H]
\centering
\caption{8 cấu hình thực nghiệm chính (cfg1 = ORIGINAL baseline bài báo).}
\label{tab:8-cfg-main}
\begin{tabularx}{\textwidth}{@{}c>{\ttfamily\raggedright}p{4.2cm}>{\ttfamily}p{2.2cm}>{\ttfamily}p{3.2cm}X@{}}
\toprule
\# & Cardinality & IC & SB & Ghi chú \\
\midrule
1 & sorting-network & none & none & ORIGINAL ― baseline bài báo \\
2 & sorting-network & none & slot-service & \\
3 & sorting-network & both & none & \\
4 & sorting-network & both & slot-service & \\
5 & totalizer & none & none & \\
6 & totalizer & none & slot-service & \\
7 & totalizer & both & none & \\
8 & totalizer & both & slot-service & \\
\bottomrule
\end{tabularx}
\end{table}

Ba họ objective được chạy thành campaign độc lập để không trộn baseline B0,
B1 và B2. Script \texttt{run\_main\_8cfg.sh} mặc định chỉ chạy
\texttt{method=weighted} trên toàn bộ 800~instances
(400~\texttt{U30} + 400~\texttt{U40}), tức $8\times800=6\,400$ top-level
runs. Script \texttt{run\_lex\_8cfg.sh} gọi cùng ma trận hai lần, một lần cho
\texttt{lex-continuity} và một lần cho \texttt{lex-overtime}, tức
$2\times8\times800=12\,800$ top-level runs; mỗi run full-coverage B1 có ba
solver calls tuần tự dùng chung một cumulative timeout. Hai policy B1 được ghi
vào hai result directory riêng, không ghi đè JSON, resume key hoặc bảng tổng
hợp của B0.

Script \texttt{run\_epsilon\_8cfg.sh} chạy năm mức $\delta$ trong
Eq.~\eqref{eq:similarity-budget} vào năm result directory riêng. Nếu chạy đủ
ma trận và 800 instances, B2 có
$5\times8\times800=32\,000$ top-level runs. Mỗi run full-coverage gọi solver
bốn lần: một lần tính $\SIM^*$ và ba lần cho
$\min\CONT\rightarrow\min\OT\rightarrow\max\SIM$; soft-coverage gọi thêm một
tầng coverage. Tầng similarity tie-break cuối là cần thiết khi
$L_\delta<\SIM^*$ để loại nghiệm có cùng continuity/overtime nhưng similarity
thấp hơn. Khi $L_\delta=\SIM^*$ tầng này là dư thừa về logic, nhưng vẫn được
giữ để mọi mức $\delta$ có cùng schema bốn tầng và chi phí đo nhất quán. Báo
cáo phải kèm \texttt{solver\_calls} và cumulative timeout để làm rõ chi phí
sequential optimization. Vì ngân sách này lớn, full campaign chỉ được mở sau
compute-budgeted pilot và go/no-go gate của WP5. Cả ba runner hỗ trợ cờ
\texttt{-{}-resume}.

Mỗi JSON ghi \texttt{objective\_mode}, \texttt{objective\_policy}, metric cuối
cùng và optimum của từng tầng trong \texttt{stages}. Với B1, thứ tự so sánh
chính là vector optimum theo policy; \texttt{weighted\_reference\_score} chỉ
được lưu để phân tích tham chiếu và không được dùng thay cho thứ tự
lexicographic. Với B2, JSON ghi thêm:
\begin{itemize}[nosep]
  \item \texttt{delta} và \texttt{similarity\_reference\_optimum};
  \item \texttt{similarity\_lower\_bound};
  \item realized similarity loss ở cả dạng tuyệt đối và phân số.
\end{itemize}
Script \texttt{collect\_main\_results.py} tổng hợp JSON thành ba file CSV:
\begin{itemize}
  \item \texttt{results\_per\_instance.csv} --- 1~dòng/run, chứa toàn bộ metrics;
  \item \texttt{summary\_by\_config.csv} --- 1~dòng/cấu hình
        (PAR-2, OPTIMUM\%, mean/median elapsed, số biến, clause);
  \item \texttt{comparison\_pivot.csv} --- 1~dòng/instance, so sánh 8~cfg.
\end{itemize}
\texttt{collect\_epsilon\_results.py} còn tạo bảng gộp toàn bộ delta, summary
theo delta/cấu hình và bảng unique objective points; các delta dẫn đến cùng
$(\COV,\SIM,\CONT,\OT)$ được gộp nhưng danh sách delta nguồn vẫn được giữ.
Khi phân tích B2, số raw delta runs, số $L_\delta$ phân biệt và số unique
objective points được trình bày đồng thời để tránh phóng đại độ phủ Pareto.
Các cột tối thiểu được chia thành năm nhóm:
\begin{itemize}[nosep]
  \item nhận dạng: \texttt{instance\_name}, \texttt{method},
        \texttt{objective\_policy}, \texttt{status};
  \item thời gian/công thức: \texttt{elapsed\_s}, \texttt{variables},
        \texttt{hard\_clauses}, \texttt{soft\_clauses}, \texttt{solver\_calls};
  \item các tầng: \texttt{stage\_objectives}, \texttt{stage\_optima};
  \item metric nghiệm: similarity, continuity, overtime và coverage;
  \item ngân sách B2: similarity reference, lower bound và realized loss.
\end{itemize}
Không tái sử dụng result directory khi schema hoặc campaign thay đổi.
...
Timeout cho thực nghiệm chính: \textbf{3600~giây} (1~giờ)/run.
Pilot screening dùng 60~giây để tiết kiệm thời gian.

\paragraph{Solver backend.}
Bài báo gốc (Collcaballero et al.) dùng \textbf{EvalMaxSAT} trên Linux
x86-64 cho toàn bộ thực nghiệm. Sau điều tra kỹ thuật,
EvalMaxSAT không tạo ra output \texttt{s OPTIMUM FOUND} trên macOS arm64
với các WCNF lớn của HCORAP (thoát bình thường sau vài cải tiến trung gian;
SIGSEGV khi gọi qua \texttt{hcorap\_multi}). Thực nghiệm này dùng
\textbf{Open-WBO} (build từ source, macOS arm64) cho tất cả 8 cấu hình.
So sánh thời gian với bài báo gốc là không hợp lệ (khác solver và platform).
Khi backend trả về \texttt{OPTIMUM}, model, objective value và các inherited
bounds được verifier C++ tính lại; tuy nhiên optimality vẫn dựa trên tính đúng
của backend vì pipeline hiện chưa kiểm tra một proof trace MaxSAT độc lập.
...
EvalMaxSAT không ổn định trên nền tảng chạy & \textbf{Đã xử lý:} EvalMaxSAT
không output \texttt{OPTIMUM} trên macOS arm64 với WCNF lớn; khóa
Open-WBO C++ đã qua smoke test cho mọi phương pháp; không thay solver giữa
campaign. So sánh runtime với bài báo gốc là không hợp lệ; chỉ so sánh
objective value. \\
...
Ma trận $2\times5\times5$ làm vượt compute budget & \textbf{Đã xử lý:} Pilot trên
4~instance xác nhận loại \texttt{both-plus} (67\% OPTIMUM) và các SB
không có ảnh hưởng ý nghĩa; cố định $2\times2\times2=8$~cấu hình
cho thực nghiệm chính. \\
