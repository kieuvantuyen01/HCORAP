...
Script \texttt{run\_main\_8cfg.sh} chạy tuần tự toàn bộ 800~instances
(400~\texttt{U30} + 400~\texttt{U40}), ghi JSON từng run vào
\texttt{cfg}$k$/\textit{instance}\texttt{.json};
hỗ trợ cờ \texttt{-{}-resume} để chạy lại từ điểm dừng.
...
\end{itemize}
Các cột tối thiểu: \texttt{instance\_name}, \texttt{status},
\texttt{elapsed\_s}, \texttt{variables}, \texttt{hard\_clauses},
\texttt{soft\_clauses}, \texttt{best\_value}, \texttt{similarity},
\texttt{continuity}, \texttt{overtime}, \texttt{coverage}.
...
so sánh objective value $\SIM - w_c\,\CONT - w_o\,p\,\OT$ là hợp lệ vì cả
hai solver chứng nhận optimality khi trả về \texttt{OPTIMUM}.
